...
Pipra_filicauda <- read.structure("Pipra_filicauda_SNPs_phased_rmIndels_75_QC_DP_random_seqcap_pop_structure.str", 
                         n.ind=7, n.loc=1018, onerowperind=FALSE, 
                         col.lab=1, col.pop=0, col.others=0, row.marknames=0)

setwd("/Volumes/Brumfield_Lab_Drive/data/1_analysis/dapc_pca/Pipra_filicauda")
Pipra_filicauda.X <- tab(Pipra_filicauda, freq=TRUE, NA.method = "mean")
Pipra_filicauda.X <- scaleGen(Pipra_filicauda, NA.method = "mean")
sum(is.na(Pipra_filicauda$tab)) #amount of missing data
Pipra_filicauda.X[1:2,1:2]
Pipra_filicauda.pca <- dudi.pca(Pipra_filicauda.X, scale=FALSE, scannf = FALSE, nf = 4)
Pipra_filicauda.pca$eig[1]/sum(Pipra_filicauda.pca$eig) * 100
Pipra_filicauda.pca$eig[2]/sum(Pipra_filicauda.pca$eig) * 100

Pipra_filicauda.pca$li

Pipra_filicauda.grp <- find.clusters(Pipra_filicauda, n.pca = 100)

xval <- xvalDapc(Pipra_filicauda.X, Pipra_filicauda.grp$grp, n.pca.max = 20, training.set = 0.9,
                 result = "groupMean", center = TRUE, scale = FALSE,
                 n.pca = NULL, n.rep = 30, xval.plot = TRUE)

dapc1 <- dapc(Pipra_filicauda.X, Pipra_filicauda.grp$grp)
...
tiff("Pipra_filicauda_colors.tiff", width = 6, height = 6, units = 'in', 
     res = 800, compression = 'none')
colorplot(Pipra_filicauda.pca$li, Pipra_filicauda.pca$li, add.plot=FALSE, cex=2, 
          xlim=c(-15, 15), ylim=c(-15, 15), 
          xlab="PC 1 (35.3%)", ylab="PC 2 (33.5%)",
          main = "PCA of Pipra filicauda 1018 SNPs \naxes 1-2") 
abline(v=0,h=0,col="grey", lty=2)
dev.off()


# pca with labeled points
tiff("Pipra_filicauda_labeled.tiff", width = 6, height = 6, units = 'in', 
     res = 800, compression = 'none')
fviz_pca_ind(Pipra_filicauda.pca,
      	     xlim=c(-15, 15), ylim=c(-15, 15), 
             repel = TRUE,
             title = "PCA of Pipra filicauda 1018 SNPs \naxes 1-2"
)
dev.off()

# pca without labeled points - groups
tiff("Pipra_filicauda_groups.tiff", width = 6, height = 6, units = 'in', 
     res = 800, compression = 'none')
fviz_pca_ind(Pipra_filicauda.pca,
             xlim=c(-15, 15), ylim=c(-15, 15),
             geom = "point",
...
tiff("Pipra_filicauda_black.tiff", width = 6, height = 6, units = 'in', 
     res = 800, compression = 'none')
fviz_pca_ind(Pipra_filicauda.pca,
             geom = "point",
             xlim=c(-15, 15), ylim=c(-15, 15), 
